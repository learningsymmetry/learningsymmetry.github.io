...
description: |
  Understanding the problem of generalization, and motivating the study of symmetry as a solution.
author:
  - name: Carles Gelada
  - name: Jacob Buckman
    url: jacobbuckman.com
    affiliation: Mila
    affiliation_url: https://mila.quebec/en/
date: 12-10-2022
output:
...
At its core, machine learning is about fitting functions to data. There is a target function $f: X\to Y$ between two spaces that we'd like to learn. The input space $X$ might be text sequences, images encoded as arrays of pixels, sound waves, etc. Similarly, the output space $Y$ could be numbers, text, images, etc.
...
|![](https://i.imgur.com/hQcvK2Em.jpg)| ![](https://i.imgur.com/7GHaeYPm.jpg)|
| ----- | -----|
...
<!---
We can put all these foundational ideas into a little interactive visualization. Below you can draw or upload any image and select what pixels should be used as part of the dataset. In the dropdown you can pick one of several neural network architectures. You'll see how the image changes as the neural network trains.

VIZ

Even though this is a very insightful way to think about machine learning, one that will keep coming back during this and other articles, it's worth noting that this is not the domain where the power of neural networks really shines. You might find the hability of the model to predict missing parts of the image underwelming. This is mostly because the amounts of data involved in training this image model are very small. To really appreciate what makes neural networks so exceptional we need to discuss their generalization capabilities.
-->
...
<!---
NOTE: Add image or JS viz showing the two regions $X_c, X_e$ in green and red respectively.
--->
...
![](https://i.imgur.com/8foVvCYm.jpg)           | ![](https://i.imgur.com/WHkWsKMm.jpg) | ![](https://i.imgur.com/hJLfOrwm.jpg)
:-------------------------:|:-------------------------:|:-------------------------:
"Photo of hip hop cow in a denim jacket recording a hit single in the studio" | "An italian town made of pasta, tomatoes, basil and parmesan" |“Spider-Man from Ancient Rome” 
...
An important fact to keep in mind is that modern ML tends to use parameter spaces $\R^p$ which are of much higher dimensionality than $|S|$, the number of datapoints in our dataset. If we are using a polynomial function space, we can use [Lagrange polynomials](https://en.wikipedia.org/wiki/Lagrange_polynomial) to show that for any function $f$, if $|S| < p$, there will always exist parameters $w\in \R^p$ with $h_w(x) = f(x)$ for all $x\in S$.  More over, gradient descent (with a reasonably defined loss function) is guaranteed to find one of these solutions. We have some similar results for neural networks: an immediate consequence of the [universal function approximation theorems](https://en.wikipedia.org/wiki/Universal_approximation_theorem) is that there exist parameters $w$ that fit the dataset if $p$ is large enough. Showing that gradient descent would *find* this solution is a different story though: as far as we know, no such results exist
^[Outside of limiting cases with specialized parameterizations, such as [NTK](https://en.wikipedia.org/wiki/Neural_tangent_kernel).]
...
In perhaps one of the most important and insightful experiments of deep learning, [Zhang et al.](https://arxiv.org/abs/1611.03530) took a variety of neural network architectures used in computer vision, and trained them to fit a few variations of ImageNet, a popular image classification dataset.
...
![](https://i.imgur.com/f19D7uAm.jpg) | ![](https://i.imgur.com/NxgqlJbm.jpg)
| -------- | -------- | 
| Image 1 | Image 2
...
![](https://i.imgur.com/UYbq6Qsm.jpg)| ![](https://i.imgur.com/0PnGGIJm.jpg)
| -------- | -------- | 
| Image 1 | Image 2
...
knitr::include_graphics("https://i.imgur.com/SWNEazy.jpg")
```
...
| Hotdog | Not hotdog | 
| -------- | -------- |
| ![](https://i.imgur.com/TeLHlHBm.jpg) | ![](https://i.imgur.com/EEArbcLm.jpg) |
| ![](https://i.imgur.com/YjaOXSfm.jpg) | ![](https://i.imgur.com/VfECaanm.jpg) |
...
| -------- | -------- |
| ![](https://i.imgur.com/Ckgsdsbm.jpeg) | ![](https://i.imgur.com/gVPqfPsm.png) |
...
The way we've been disussing how one might gain confidence in the predictions of a neural network from symmetry would seem to have some very strong limitations. From example a neural network trained to read a multi digit number in an image. It might seem that could only ever be confident about the predictions of the nerual network for images containing digts that we've seen in the dataset. This limitation is just a product of our policy of picking simplicity over generality in the aricle. Our definition of a funciton having a symmetry really should be called having an invariance. There is a more general interaction of grups an funcitons call equivariance which can handle this problem. We'll get to it at some point but for now there is a lot of work to be done just in this simpler case.

What I haven't done. Show that deep learnig magic occurs in this idealized setting. In practice the implementations are not the same as the one we're studying. 
--->
